...
def test_clifford_t_set_transpiler_named_angles() -> None:
    b = SubBuilder(2)
    q0, q1 = b.qubits
    b.add_op(H, (q0,))
    b.add_op(RZ(pi / 4), (q0,))
    ...
    compiled = compile_sub(transpiled, CliffordT, sub_transpilers=())
    circuit = Evaluator(QURIPartsEvaluatorHooks()).run(compiled)
    assert {gate.name for gate in circuit.gates} <= _ALLOWED


@pytest.mark.gridsynth
def test_clifford_t_set_transpiler_arbitrary_angle_approximated() -> None:
    b = SubBuilder(2)
    q0, q1 = b.qubits
    b.add_op(H, (q0,))
    b.add_op(RZ(0.3), (q0,))
    ...
    # A non-pi/4 angle is approximated into {H, S, T} with gridsynth, so no RZ
    # survives and the result lands entirely in the Clifford+T set.
    transpiled = CliffordTSetTranspiler(epsilon=1e-3)(sub)
    compiled = compile_sub(transpiled, CliffordT, sub_transpilers=())
    circuit = Evaluator(QURIPartsEvaluatorHooks()).run(compiled)
