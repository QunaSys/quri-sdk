...
class VM:
    """Executor and analyzer of quantum programs abstracting architectures and
    devices.
    ...
    def from_device_prop(prop: DeviceProperty) -> "VM":
        """Utility method, which creates an instance of a VM from a
        DeviceProperty in QURI Parts."""
    ...
    def estimate(
        ...
        """Estimate expectation value of an operator for the state generated by
        a quantum circuit. VM itself is an Estimator of QURI Parts.
    ...
    def sample(
        ...
        """Sample the given circuit at VM's lowering level. VM itself is an
        Sampler of QURI Parts.
    ...
    def transpile(
        ...
        """Transpile the given circuit to adapt with the target architecture
        and device at current lowering level.

        Transpilation is performed automatically during execution and
        analysis, but users can also use this method to consciously
        transpile circuits.
        """
    ...
    def analyze(self, circuit: NonParametricQuantumCircuit) -> AnalyzeResult:
        """Analyze and estimate the costs of the given circuit at current
        lowering level."""
        if self._lowering_level == LoweringLevel.LogicalCircuit:
